...
export default function QuizCreateDialog({ open, onClose, onCreated }: Props) {
    ...
    const readFile = (file: File) => {
        if (file.type === 'text/plain' || file.name.endsWith('.txt')) {
            ...
            // For PDF: use FileReader to get text from simple PDFs
            // We convert to text via a basic approach
            const reader = new FileReader()
            reader.onload = (e) => {
                // Extract readable text from PDF binary (basic heuristic)
            ...
            toast('Please upload a .txt or .pdf file', { className: '!bg-red-100 !rounded-2xl' })
        }
    }
