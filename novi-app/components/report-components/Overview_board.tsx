            ...
            <p className="text-white/80 text-[15px] font-medium mb-8">A quick overview of your previous classes' performance and aggregated metrics.</p>
                    ...
                    <div className="text-white text-base w-full">
                        ...
                        <div className="bg-[#ecfeff] w-max px-6 py-3 rounded-2xl mb-8 flex items-center gap-10 font-bold shadow-md">
                            ...
                            <span className="text-[#43a5d1]/80 text-sm">Date: {sessionDate ? (
                                <span className="text-[#43a5d1]">{sessionDate}</span>
                            ) : (
                                <span className="text-[#43a5d1]/50">No recent sessions</span>
                            )}</span>
                            {sessionTime && (
                                <span className="text-[#43a5d1]/80 text-sm">Time: <span className="text-[#43a5d1]">{sessionTime}</span></span>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col bg-[#ecfeff] rounded-[24px] shadow-sm overflow-hidden w-full">
                            <div className="bg-[#ecfeff] px-6 py-4 flex flex-wrap items-center gap-6 border-b border-white/10">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="bg-[#43a5d1] rounded-lg flex items-center px-1">
                                        <input 
                                            type="number" 
                                            value={threshold}
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-[#43a5d1] hover:bg-[#328eb8] transition-colors text-white font-bold text-sm"
                                >
                            ...
                            <div className="p-6 flex flex-col gap-4 min-h-[250px] w-full">
                                ...
                                {loading && <div className="text-[#43a5d1]/70 font-bold animate-pulse text-center py-16 flex-1 flex flex-col items-center justify-center">Loading data...</div>}
                                
                                {/* Empty state indicator when no records match filter criteria */}
                                {!loading && distractions.length === 0 && (
                                    <div className="flex-1 flex flex-col items-center justify-center text-[#43a5d1]/90 font-bold text-center py-24 px-8 w-full bg-white/[0.04] rounded-xl border border-white/5 shadow-inner">
                                        {threshold === '' 
                                ...
                                {!loading && distractions.map((d, i) => (
                                    <div key={i} className="bg-[#43a5d1] p-5 rounded-2xl flex justify-between items-center shadow-sm border border-white/5">
